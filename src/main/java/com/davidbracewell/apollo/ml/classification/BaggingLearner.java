...
public class BaggingLearner extends ClassifierLearner {
   ...
   */
  public BaggingLearner(SerializableSupplier<ClassifierLearner> learnerSupplier, int numberOfBags, int bagSize) {
  ...
  protected Classifier trainImpl(Dataset<Instance> dataset) {
    ...
    for (int i = 0; i < numberOfBags; i++) {
      model.models.add(
        learnerSupplier.get().train(dataset.sample(true, targetBagSize))
      );
    }
    return model;
  }
