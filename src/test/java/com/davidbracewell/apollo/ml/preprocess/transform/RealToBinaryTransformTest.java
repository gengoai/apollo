 ...
 */
public class RealToBinaryTransformTest extends BaseInstancePreprocessorTest {
   @Test
   public void test() throws Exception {
      ...
      assertEquals(1, ii.getValue("sepal_length"), 0.01);

      transform = new RealToBinaryTransform(3);
      ds = getData(transform);
      ds.encode();
      featureNames = ds.getFeatureEncoder().values();
      assertTrue(featureNames.contains("petal_length"));
      assertFalse(featureNames.contains("petal_width"));
      assertTrue(featureNames.contains("sepal_length"));
      assertTrue(featureNames.contains("sepal_width"));

      ii = ds.stream().first().orElse(Instance.create(Collections.emptyList()));
      assertEquals(1, ii.getValue("sepal_length"), 0.01);
      assertEquals(1, ii.getValue("sepal_width"), 0.01);
      assertEquals(0, ii.getValue("petal_length"), 0.01);
      assertEquals(0, ii.getValue("petal_width"), 0.01);
   }
